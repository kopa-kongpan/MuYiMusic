...
interface VideoCourseBindingFormValue {
...
interface ProductFormValues {
  ...
  sort_order: number
  skus: SkuFormValue[]
  video_course_bindings: VideoCourseBindingFormValue[]
}
...
export function ProductFormModal({
  ...
  const videoCourseBindings =
    Form.useWatch('video_course_bindings', form) ?? []
  const videoCoursesQuery = useQuery({
    queryKey: ['admin-active-video-courses', store.id],
    queryFn: () =>
      apiClient.listAdminVideoCourses(store.id, {
        isActive: true,
        pageSize: 100,
      }),
    enabled: open,
  })
  const videoCourses = videoCoursesQuery.data?.items ?? []
  ...
  async function uploadFile(file: File, purpose: 'product') {
  ...
  async function save() {
    ...
    const payload: ProductCreate = {
      ...
      video_course_bindings: values.video_course_bindings,
    }
    setIsSaving(true)
    try {
      if (product) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="sku-heading">
          <strong>配套视频课程</strong>
          <span>可绑定多个视频课程，并开放全部或指定课时</span>
        </div>
        <Form.List name="video_course_bindings">
          {(fields, { add, remove }) => (
            <div className="chapter-list">
              {fields.map((field, index) => {
                const courseId =
                  videoCourseBindings[field.name]?.video_course_id
                const mode = videoCourseBindings[field.name]?.access_mode
                const videoCourse = videoCourses.find(
                  (course) => course.id === courseId,
                )
                return (
                  <div className="chapter-row" key={field.key}>
                    <Form.Item
                      name={[field.name, 'video_course_id']}
                      label="视频课程"
                      rules={[{ required: true, message: '请选择视频课程' }]}
                    >
                      <Select
                        options={videoCourses.map((course) => ({
                          value: course.id,
                          label: `${course.name}（${course.lessons.filter((lesson) => lesson.is_active).length} 课时）`,
                        }))}
                        onChange={() =>
                          form.setFieldValue(
                            ['video_course_bindings', field.name, 'lesson_ids'],
                            [],
                          )
                        }
                      />
                    ...
                    <Form.Item
                      name={[field.name, 'access_mode']}
                      label="开放范围"
                      rules={[{ required: true }]}
                    >
                      <Select
                        options={[
                          { value: 'all', label: '全部课时' },
                          { value: 'selected', label: '指定课时' },
                        ]}
                        onChange={(value) => {
                          if (value === 'all') {
                            form.setFieldValue(
                    ...
                    {mode === 'selected' ? (
                      <Form.Item
                        name={[field.name, 'lesson_ids']}
                        label="开放课时"
                        rules={[{ required: true, message: '请选择开放课时' }]}
                      >
                        <Select
                          mode="multiple"
                          options={(videoCourse?.lessons ?? [])
                            .filter((lesson) => lesson.is_active)
                            .map((lesson) => ({
                              value: lesson.id,
                              label: `第 ${lesson.lesson_number} 课时 · ${lesson.title}`,
                            }))}
                        />
                      </Form.Item>
                    ) : null}
                    <Button
                      ...
                      danger
                      icon={<Trash2 size={17} aria-hidden="true" />}
                      aria-label={`移除第 ${index + 1} 个视频课程绑定`}
                      onClick={() => remove(field.name)}
                    />
                  </div>
                )
              ...
              <Button
                block
                type="dashed"
                disabled={!videoCourses.length}
                icon={<Plus size={17} aria-hidden="true" />}
                onClick={() =>
                  add({
                    video_course_id: undefined,
                    access_mode: 'all',
                    lesson_ids: [],
                  })
                }
